<x-app-layout>
    <div class="py-12 bg-[#f8fafc] min-h-screen font-sans">
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <form id="main-form" action="{{ route('penilaian.update', $penilaian->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')

                <div class="mb-8 px-2">
                    <h2 class="text-2xl font-bold text-slate-800 tracking-tight">Edit Penilaian</h2>
                    <p class="text-slate-500 text-sm mt-1">Perbarui data parameter penilaian unit kelas yang sudah ada.</p>
                </div>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-12 divide-y lg:divide-y-0 lg:divide-x divide-slate-100">
                            ...
                            <div class="space-y-6">
                                <div>
                                    ...
                                    <div class="space-y-4">
                                        ...
                                        <div class="space-y-1.5">
                                            ...
                                            <input type="date" name="tanggal_penilaian" value="{{ $penilaian->tanggal_penilaian->format('Y-m-d') }}" required 
                                ...
                                <div class="pt-2">
                                    ...
                                    <div onclick="document.getElementById('foto-input').click()" 
                                        ...
                                        <div id="placeholder-content" class="text-center p-4 {{ $penilaian->foto ? 'hidden' : '' }}">
                                            ...
                                            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-tight">Klik Ganti Foto</p>
                                        </div>

                                        <img id="preview-img" 
                                             src="{{ $penilaian->foto ? asset('storage/'.$penilaian->foto) : '#' }}" 
                                             class="absolute inset-0 w-full h-full object-cover {{ $penilaian->foto ? '' : 'hidden' }} group-hover:opacity-90 transition-opacity">
                                    </div>
                                </div>
                            </div>
                        </div>

                        {{-- SISI KANAN: Parameter Skor --}}
                        <div class="lg:col-span-8 p-8 flex flex-col">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                                ...
                                @php
                                    $detail = $penilaian->details->where('kriteria_id', $kr->id)->first();
                                    $skor = $detail ? $detail->skor : 0;
                                @endphp
                                <div class="p-4 rounded-2xl bg-white border border-slate-100 hover:border-indigo-200 hover:shadow-sm transition-all group">
                                    <div class="flex items-center justify-between gap-4">
                                        ...
                                        <input type="number" name="nilai[{{ $kr->id }}]" min="0" max="25" value="{{ $skor }}" required
                            ...
                            <div class="space-y-2 mb-8">
                                ...
                                <textarea name="catatan" rows="3" placeholder="Tambahkan keterangan jika diperlukan..."
                                    class="w-full border-slate-200 rounded-2xl focus:ring-4 focus:ring-indigo-500/5 focus:border-indigo-500 text-sm py-3 px-4 resize-none transition-all bg-slate-50/30">{{ $penilaian->catatan }}</textarea>
                            </div>
                ...
                <p class="text-center mt-6 text-[11px] text-slate-400 font-medium italic opacity-60">
                    * Perubahan data akan langsung memperbarui kalkulasi skor akhir di sistem.
                </p>
            </form>
        </div>
    </div>
